...
(defun list-endp-maker-forward (end)
  (lambda (list it)
    (declare (ignore list))
    (eq it end)))
...
(defmethod make-iterator ((object list) &key (start 0) end from-end)
  ...
  (let ((lstart (nthcdr start object)))
    (values lstart
            #'list-step-forward
            (list-endp-maker-forward (if end (nthcdr (- end start) lstart) nil))
...
(defun vector-endp-maker (n)
  (declare (type (integer -1 (#.array-dimension-limit)) n))
  (lambda (vector it)
    (declare (ignore vector)
             (type (integer -1 (#.array-dimension-limit)) it))
    (= it n)))
...
(defmethod make-iterator ((object vector)
  ...
  (values (if from-end (1- end) start)
          ...
          (vector-endp-maker (if from-end (1- start) end))
          #'vector-elt #'(setf vector-elt)))
